...
public class SystemLogAspect {
	...
	// Controller层切点 AfterReturn
	@Pointcut("@annotation(com.bank.annotation.SystemControllerAfterReturnLog)")
	public void controllerAfterReturnAspect() {
	}
	...
	public void doBefore(JoinPoint joinPoint) throws Exception {
		...
		try {
			...
			// getControllerMethodDescription(joinPoint,0));
	...
	public void doAfter(JoinPoint joinPoint) throws Exception {
		...
		try {
			...
			// joinPoint.getSignature().getName() + "()"));
			// System.out.println("方法描述:" +
			// getControllerMethodDescription(joinPoint,1));
			// System.out.println("请求人:" + user.getUser_name());
			// System.out.println("请求IP:" + ip);
			// *========数据库日志=========*//
			LogModel log = new LogModel();
	...
	@AfterReturning(value = "controllerAfterReturnAspect()",argNames = "retVal", returning = "retVal")
	public void doAfterReturning(JoinPoint joinPoint, Object retVal) {
		...
		try {
			...
			// System.out.println("请求IP:" + ip);
			// *========数据库日志=========*//
			JSONObject ret = (JSONObject) retVal;
			LogModel log = new LogModel();
			if (ret.getInteger("error")==200) {
				log.setLog_msg(getControllerMethodDescription(joinPoint, 2)+":"+ret.get("name")+",登陆成功!");
			}else{
				log.setLog_msg(getControllerMethodDescription(joinPoint, 2)+":"+request.getParameter("user_account")+"登陆失败,原因<br>"+ret.get("msg"));
			}
			log.setLog_type(1);
			log.setLog_method(
					(joinPoint.getTarget().getClass().getName() + "." + joinPoint.getSignature().getName() + "()"));
			if (user==null) {
				log.setLog_user_name(request.getParameter("user_account"));
			}else{
				log.setLog_user_id(user.getUser_id());
				log.setLog_user_name(user.getUser_name());
			}
	 ...
	 *            切点
	 * @param state
	 *            0 before 1 after 2 afterReturn
	 * @return 方法描述
	 * @throws Exception
	 */
			...
			if (method.getName().equals(methodName)) {
				...
				if (clazzs.length == arguments.length) {
					...
					case 1:
						...
						break;
					case 2:
						description = method.getAnnotation(SystemControllerAfterReturnLog.class).description();
						break;
					default:
						break;
					}
