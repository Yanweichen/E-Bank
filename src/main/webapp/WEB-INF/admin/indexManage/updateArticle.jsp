...
<div class="row" style="padding: 20px">
	<div class="col-sm-6">
		    ...
		    <div class="input-group">
		      <div class="input-group-btn">
		        ...
		        <ul class="dropdown-menu">
		          ...
		          <li id="notice" ><a href="javascript:void(0)" class="active">公告</a></li>
		        </ul>
		      </div><!-- /btn-group -->
	  ...
	  <div class="form-group">
			<button id="headimage" type="button" class="btn btn-primary btn-lg btn-block">更换预览图片</button>
	  </div>
	  <div class="form-group">	
	  		<img id="selectedimg" style="height: 200px;width: 200px;" class="img-rounded">
	  		<img id="oldimg" style="height: 200px;width: 200px;" class="img-rounded">
	  </div>
	<!-- 富文本编辑器 -->
	<div class="row">
		<div class="col-sm-12" >
	...
	var art_id = <% out.print(request.getParameter("id")==null?"":request.getParameter("id")); %>;
	...
	$("#submit").click(function () {
		...
		$.post("index/updateart.action",from(),function(result){
			$(".fakeloader").fakeLoader({
		            spinner:"spinner2",
		            show:false
		        });
	...
	$(document).ready(function(){
		...
		$.getJSON("index/getArticleById.action", {id:art_id},function(json){
				$('#label').tokenfield('setTokens', json.index_label);//设置标签
				$('#editor').append(json.index_content);//设置内容
				if (json.index_from!=null) {
					$("#frombtn").empty();
					$("#frombtn").append("来自其他网站");
					$("#fromdiv").css("display","inline")
					$("#index_from").val(json.index_from);
					$("#index_href").val(json.index_href);
				}
				$("#title").val(json.index_title);
				$("#oldimg").attr("src",json.index_preview_image_url);
